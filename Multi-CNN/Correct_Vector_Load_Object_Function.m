function [VObjects]=Correct_Vector_Load_Object_Function(QP_Classifier, OptComm_Classifier, VObjects)
     
filename_Filename=strcat('DataFiles/filenames_3D_',OptComm_Classifier,'_QP',num2str(QP_Classifier));
filename_Classifier=strcat('DataFiles/correct_vec_3D_',OptComm_Classifier,'_QP',num2str(QP_Classifier));

QP=QP_Classifier;
fprintf('Loading Classification 3D Vectore for QP=%2d',QP);
%% Load file contents ...
...
for i=1:length(VObjects)
    for j=1:length(Classified)
        ...
        if strcmp(VObjects(i).Filename(1:Len),FNames(j,1:Len))
            VObjects(i).Classified(L)=Classified(j);
        end
    end
end
fprintf(' ....... Done \n');
