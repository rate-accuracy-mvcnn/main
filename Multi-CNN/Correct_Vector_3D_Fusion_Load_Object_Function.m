function [VObjects]=Correct_Vector_3D_Fusion_Load_Object_Function(QP_Classifier, OptComm_Classifier, VObjects)

%QP_Encoder=40; OptComm_Encoder='A';
%QP_Classifier=QP_Encoder;OptComm_Classifier=OptComm_Encoder;


filename_Filename=strcat('DataFiles/filenames_3D_Fusion_',OptComm_Classifier,'_QP',num2str(QP_Classifier));
filename_Classifier=strcat('DataFiles/correct_vec_3D_Fusion_',OptComm_Classifier,'_QP',num2str(QP_Classifier));

QP=QP_Classifier;
fprintf('Loading Classification 3D Fusion Vectore for QP=%2d',QP);
%% Load file contents ...
...
for i=1:length(VObjects)
    for j=1:length(Classified)
        ...
        if strcmp(VObjects(i).Filename(1:Len),FNames(j,1:Len))
            VObjects(i).Classified_3D_Fusion(L)=Classified(j);
        end
    end
end
